...
const StatCard = ({
  ...
  trendValue
    ...
    <CardHeader className="flex flex-row items-center justify-between pb-2 bg-zinc-900">
    ...
    <CardContent className="bg-zinc-900">
      ...
      {trend && <div className={`mt-2 flex items-center text-xs ${trend === 'up' ? 'text-green-500' : 'text-red-500'}`}>
...
export default function CampaignAnalytics({
  ...
  useEffect(() => {
    setAnalyticsData(generateAnalyticsData(selectedCampaignId === "all" ? undefined : selectedCampaignId, dateRange));
  ...
  const maxDataPoints = dateRange.from && dateRange.to && Math.ceil((dateRange.to.getTime() - dateRange.from.getTime()) / (1000 * 60 * 60 * 24)) > 14 ? 14 : analyticsData.length;
  ...
  return <div className="mb-8 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 w-full sm:w-auto">
          <Select value={selectedCampaignId} onValueChange={setSelectedCampaignId}>
            ...
            <SelectContent>
              ...
              {campaigns.map((campaign: Campaign) => <SelectItem key={campaign.id} value={campaign.id || ""}>
                  {campaign.title}
                </SelectItem>)}
            </SelectContent>
          </Select>
              ...
              <PopoverContent className="w-auto p-0" align="end">
                <div className="p-2 border-b">
                  <div className="grid grid-cols-1 gap-2">
                    <Button variant={selectedDatePreset === 'wtd' ? 'default' : 'ghost'} onClick={() => handleDatePresetChange('wtd')} size="sm" className="justify-start">
                      Week to date
                    </Button>
                    <Button variant={selectedDatePreset === 'mtd' ? 'default' : 'ghost'} onClick={() => handleDatePresetChange('mtd')} size="sm" className="justify-start">
                      Month to date
                    </Button>
                    <Button variant={selectedDatePreset === 'ytd' ? 'default' : 'ghost'} onClick={() => handleDatePresetChange('ytd')} size="sm" className="justify-start">
                      Year to date
                    </Button>
                    <Button variant={selectedDatePreset === 'lastMonth' ? 'default' : 'ghost'} onClick={() => handleDatePresetChange('lastMonth')} size="sm" className="justify-start">
                      Last month
                    </Button>
                  </div>
                </div>
                <div className="p-2">
                  ...
                  from: dateRange.from,
                  to: dateRange.to
                }} onSelect={handleCalendarSelect} numberOfMonths={2} defaultMonth={dateRange.from} className="p-3 pointer-events-auto rounded-md border" />
                </div>
              </PopoverContent>
            </Popover>
          </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <StatCard title="Total Views" value={totalViews.toLocaleString()} description={formatDateRange(dateRange)} icon={<Eye className="h-4 w-4" />} trend="up" trendValue="+12.5%" />
        <StatCard title="Average CPM" value={`$${avgCPM.toFixed(2)}`} description="Cost per thousand views" icon={<TrendingUp className="h-4 w-4" />} trend="down" trendValue="-3.2%" />
        <StatCard title="Total Spent" value={`$${totalSpent.toFixed(2)}`} description={formatDateRange(dateRange)} icon={<DollarSign className="h-4 w-4" />} trend="up" trendValue="+8.7%" />
      </div>
            ...
            <div className="h-[200px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                ...
                bottom: 5
                  ...
                  return typeof value === 'number' ? `${value.toLocaleString()} views` : value;
                }} />
