...
import { 
  BarChart3, Home, Compass, Users, MessageSquare, 
  LogOut, Moon, Sun, Menu, X, Settings, Wrench, Building2, User, Briefcase
} from "lucide-react";
import { 
...
const menuItems = [
  ...
  { title: "Workspace", path: "/dashboard/profile/workspace", icon: Briefcase },
  { title: "Explore", path: "/dashboard/profile/explore", icon: Compass },
  { title: "Tools", path: "/dashboard/profile/tools", icon: Wrench },
  { title: "Profile", path: "/dashboard/profile/settings", icon: User }
];
...
const DashboardToggle = () => {
  ...
  const switchDashboard = () => {
    localStorage.setItem('selectedDashboard', 'business');
  ...
  return (
    <Button 
      ...
      className="h-8 px-3 bg-background/80 backdrop-blur-md hover:bg-blue-50 hover:border-blue-200 transition-all"
    >
            ...
            {state === "expanded" && (
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2">
                  <Badge variant="secondary" className="text-xs bg-green-100 text-green-700">Creator</Badge>
                </div>
              </div>
            )}
          </div>
